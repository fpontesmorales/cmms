...
def consultar_chamado_view(request):
    ...
    email_buscado = None
    form_email = ConsultaChamadoForm(request.POST or None)

    if request.method == 'POST' and 'buscar_email' in request.POST:
    ...
    if email_buscado:
        ...
        filter_form = PublicChamadoFilterForm()

    contexto = {
        'form_email': form_email if request.POST else ConsultaChamadoForm(initial={'email_solicitante': email_buscado}),
...
def chamado_detalhe_view(request, chamado_id):
    ...
    contexto = {'chamado': chamado, 'form_interacao': form_interacao, 'pode_comentar': pode_comentar, 'form_avaliacao': form_avaliacao, 'form_reabertura': form_reabertura}
