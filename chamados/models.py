...
class Chamado(models.Model):
    STATUS_CHOICES = [
        ('ABERTA', 'Aberta'), ('DESIGNADO', 'Designado'), ('EM_ANALISE', 'Em Análise'),
        ('PENDENTE', 'Pendente'), ('EM_EXECUCAO', 'Em Execução'),
    ...
    def get_whatsapp_message(self):
        tecnico_info = "Não atribuído"
        if self.tecnico_responsavel:
            tecnico_info = self.tecnico_responsavel.get_full_name() or self.tecnico_responsavel.username
        
        texto_whatsapp = f"""*CHAMADO DE MANUTENÇÃO*

*Número:* #{self.id}
*Status:* {self.get_status_display()}
*Serviço:* {self.tipo_servico.nome}
*Local:* {self.sala}
*Ativo:* {self.ativo or 'N/A'}
*Técnico Designado:* {tecnico_info}
